...
const getAIClient = () => {
    // Menggunakan GEMINI_API_KEY sesuai permintaan
    const apiKey = process.env.GEMINI_API_KEY;
    
    if (!apiKey) {
        console.error("GEMINI_API_KEY is missing in environment variables.");
        throw new Error("Konfigurasi API Key belum terpasang. Pastikan GEMINI_API_KEY ada di file .env");
    }
  ...
  try {
    ...
    if (modelId === ModelType.BANANA_PRO_3) {
        ...
        if (candidate?.content?.parts) {
            ...
            outputMetadata = candidate.groundingMetadata as unknown as GroundingMetadata;
        }
    ...
    if (error.message?.includes("PERMISSION_DENIED") || error.status === 403) {
         return { text: "⚠️ **Akses Ditolak**. API Key tidak valid atau tidak memiliki izin akses ke model ini. Mohon periksa file .env Anda." };
    }
    
    if (error.message?.includes('429') || error.message?.includes('Quota exceeded')) {
        return { text: "⚠️ **Kuota Habis**. Mohon tunggu sebentar sebelum mengirim pesan lagi." };
    }
